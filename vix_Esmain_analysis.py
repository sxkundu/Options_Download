"""
VIX & ES Futures Expected Move Calculator
------------------------------------------
Fetches live VIX and ES (E-mini S&P 500 front-month futures) data,
...
def get_es():
    """Return ES front-month futures value and change from previous close."""
    data = fetch_quote("ES=F")
    es = data["price"]
    prev = data["prev_close"]
    change = es - prev
    pct_change = (change / prev) * 100
...
def main():
    ...
    print("  📊  VIX & ES FUTURES EXPECTED MOVE CALCULATOR")
    ...
    # ── ES Futures ────────────────────────────────────────────
    print("\n[ ES — E-mini S&P 500 Front-Month Futures ]")
    print_separator()
    es = get_es()
    print(f"  Current :  {es['current']:,.2f}")
    print(f"  Prev Close:{es['prev_close']:,.2f}")
    print(f"  Change  :  {format_change(es['change_pts'])} pts  "
          f"({format_change(es['change_pct'])}%)")
    ...
    for label, days in [("Daily (1 day)", 1), ("Weekly (5 days)", 5),
                        ("Monthly (21 days)", 21)]:
        em = expected_move(es["current"], vix["current"], days)
    ...
    print_separator()
    em_1d = expected_move(es["current"], vix["current"], 1)
    sd_levels = std_deviation_levels(es["current"], em_1d["points"])
    ...
    print_separator(width=80)
    em_1w = expected_move(es["current"], vix["current"], 5)
    sd_levels_w = std_deviation_levels(es["current"], em_1w["points"])
