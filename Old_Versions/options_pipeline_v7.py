...
# ─────────────────────────────────────────────
def _normalize_chain(df_raw: pd.DataFrame) -> pd.DataFrame:
    """
    Convert all numeric columns in a yfinance option chain DataFrame to
    plain float64 BEFORE iterrows() is called.

    yfinance returns volume / openInterest as pandas nullable Int64.
    When iterrows() yields a row that contains pd.NA in an Int64 column,
    pandas internally tries to cast pd.NA to a Python int, which raises:
        "cannot convert float NaN to integer"
    The fix is .astype(object) first (breaks the Int64 extension type),
    then pd.to_numeric(..., errors="coerce") which gives a plain float64
    column where missing values are np.nan, never pd.NA.
    """
    df = df_raw.copy()
    for col in ["strike", "volume", "openInterest", "impliedVolatility",
                "bid", "ask", "lastPrice", "change", "percentChange"]:
...
def fetch_options(ticker_symbol: str) -> pd.DataFrame:
    ...
    for exp_str in expirations:
        ...
        for opt_type, df_raw in [("call", chain.calls), ("put", chain.puts)]:
            # Normalize BEFORE iterrows() - converts nullable Int64 -> float64
            # so every value is a plain Python float (np.nan for missing)
            df = _normalize_chain(df_raw)
            ...
            for _, row in df.iterrows():
                ...
                # Volume filter — volume is now either an int or None, never pd.NA
