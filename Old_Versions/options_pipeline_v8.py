...
def insert_data(conn, df: pd.DataFrame):
    ...
    # Use df.to_dict("records") — completely avoids iterrows/itertuples
    # and any nullable-type casting issues; values are already Python-native
    # types set by _clean_chain (Decimal, int, None, date).
    records = [
        (
            str(r["ticker"]),
            r["as_of_date"],
            r["expiry_date"],
            str(r["option_type"]),
            r["strike_price"],       # Decimal(18,4) or None
            r["volume"],             # int or None
            r["open_interest"],      # int or None
            r["implied_vol"],        # Decimal(10,6) or None
            r["spot_price"],         # Decimal(18,4)
            r["delta"],              # Decimal(10,6) or None
            r["theta"],              # Decimal(10,6) or None
        )
